...
@pytest.mark.parametrize("dtype", [torch.float32]) # torch.float16 sum reductions don't compile at the moment
def test_layer_norm(num_cols, num_threads, dtype):
    print(f"Running with num-cols: {num_cols}, "
    ...
    reference = torch.nn.functional.layer_norm(x,
                                               (num_cols,),
                                               weight=weights,
                                               bias=biases,
                                               eps=EPSILON)
