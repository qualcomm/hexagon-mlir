...
# test_softmax_torch.py
# This test creates a graph containing a single node - a Softmax op
...
def test_softmax_torch(enablelwp, enableetm):
    ...
    manager.execute_and_compare(
        rtol=5e-03, atol=1e-05, dump_outputs=True, options=options, enable_etm=enableetm
    )
