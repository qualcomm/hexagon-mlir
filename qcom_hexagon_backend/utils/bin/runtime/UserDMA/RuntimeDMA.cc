//=== dma_runtime.cc - DMA APIs for users ===//
/// The Hexagon User DMA hardware allows users to have low level
/// controls over cache behaviour during DMA transfers. It is
/// possible to avoid cache actions for enforcing coherency by
/// programming the "bypassCache" bits. It is possible for the
/// Hexagon hardware to check if a memory location is modified
/// by other hardware units (eg. DMA) and if so, to mark cache line
/// for that location, if present in the cache, as invalid (stale).
/// This "snooping" could cost power and time. If the programmer
/// is sure that the write location is absent
/// in cache (eg. that location is mapped to VTCM), this 'snoop
/// and invalidate' action is redundant. We can prevent this redundant
/// effort using the "bypassCache" flag (we can specify bypass cache
/// options - details in the API description). This optimization should
/// be used carefully, or data coherence could be hit.
