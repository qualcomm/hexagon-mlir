...
createPackOp(RewriterBase &rewriter, Location &loc, Value input,
  ...
  Value emptyPackedImage = rewriter.create<tensor::EmptyOp>(
      loc, packedType.getShape(), packedType.getElementType());
  return rewriter.create<linalg::PackOp>(loc, input, emptyPackedImage, dimsPos,
                                         tiles, padValue);
}
...
static mlir::linalg::UnPackOp createUnpackOp(RewriterBase &rewriter,
  ...
  Value emptyOrigImage = rewriter.create<tensor::EmptyOp>(
      loc, origShape, inputType.getElementType());
  return rewriter.create<linalg::UnPackOp>(loc, input, emptyOrigImage, dimsPos,
                                           tiles);
}
